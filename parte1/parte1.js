// Script para renderizar a pirâmide
...
function loadShader(gl, type, source) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        alert(
            "Um erro ocorreu ao compilar o shader: " +
...
function initBuffers(gl) {
    // Cria um buffer para as posições dos vértices da pirâmide.
    const positionBuffer = gl.createBuffer();

    // Selecione o positionBuffer para aplicar as operações de buffer.
    gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);

    // Posições dos vértices da pirâmide.
    const positions = [
        // Base da pirâmide
        -1.0, -1.0, 0,
        1.0, -1.0, 0,
    ...
    // Definindo uma cor para cada face do cubo (de forma "aleatória")
    const faceColors = []

    for (let i = 0; i < 64; i++) {
        var random_color = (9 * Math.random() + 1) / 10;
        var rounded = Math.round(random_color * 10) / 10;
        faceColors.push(rounded)
    }
...
function drawScene(gl, programInfo, buffers, deltaTime) {
    ...
    mat4.rotate(
        ...
        [0, 1, 0]
    ); // eixo para ser girar em volta

    mat4.rotate(
        modelViewMatrix, // matriz de destino
        modelViewMatrix, // matriz para rotacionar
        -Math.PI / 2, // quantidade de radianos a se rotacionar * 0.7
        [1, 0, 0]
    ); // eixo para ser girar em volta
